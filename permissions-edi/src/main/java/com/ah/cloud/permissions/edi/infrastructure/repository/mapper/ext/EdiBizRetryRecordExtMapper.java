 ...
 **/
@Mapper
public interface EdiBizRetryRecordExtMapper {
     ...
     */
    @Select("<script>"
            + " SELECT MAX(id) FROM " + RetryResourceManager.DEFAULT_RETRY_RECORD_TASK_LOGIC_TABLE_NAME + " " +
            "WHERE "
     ...
     */
    @Select("<script>"
            + " SELECT MIN(id) FROM " + RetryResourceManager.DEFAULT_RETRY_RECORD_TASK_LOGIC_TABLE_NAME + " " +
            "WHERE "
            + " record_status in (3,0) "
            + " AND deleted = 0 "
            + " AND env = #{env} "
    ...
    @Select("<script>"
            ...
            "env FROM " + RetryResourceManager.DEFAULT_RETRY_RECORD_TASK_LOGIC_TABLE_NAME + " WHERE "
            + " record_status in (3,0) "
            + " AND deleted = 0 "
            + " AND env = #{env} "
