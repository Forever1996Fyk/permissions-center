 ...
 **/
@Slf4j
public class BizRetryShardingAlgorithm implements PreciseShardingAlgorithm<String> {
    ...
    public String doSharding(Collection<String> collection, PreciseShardingValue<String> shardingValue) {
        try {
            ...
            int tableSuffix = Integer.parseInt(keys[1]) % retryConfiguration.getRetryRecordActualTableSuffixList().size();
            return retryConfiguration.getRetryRecordActualTableName() + SEPARATOR_UNDERLINE + keys[0] + SEPARATOR_UNDERLINE + tableSuffix;

        } catch (Exception e) {
            log.error("BizRetryShardingAlgorithm[doSharding] error, shardingValue is {}, reason is {}", shardingValue.getValue(), Throwables.getStackTraceAsString(e));
            throw e;
        }
    }
}
