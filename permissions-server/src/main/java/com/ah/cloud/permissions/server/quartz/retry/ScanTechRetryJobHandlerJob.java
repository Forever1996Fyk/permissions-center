...
@Slf4j
@Component
public class ScanTechRetryJobHandlerJob implements BasicProcessor {
    @Resource
    ...
    public void process(TaskContext context) {
        ...
        log.info("ScanTechRetryJobHandlerJob start handle, dateMonth is {}, jobId is {}, taskId is {}", dateMonth, context.getJobId(), context.getTaskId());
        EdiResult<Void> result = techEdiRetryRecordService.scanRetry(dateMonth, null, null, 0, 1);
        log.info("ScanTechRetryJobHandlerJob end handle, result is {}", result);
    }
}
